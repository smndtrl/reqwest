...
impl From<types::InputStream> for Body {
    #[inline]
    fn from(s: types::InputStream) -> Body {
        Body {
            kind: Some(Kind::Reader(Box::new(WasiInputStream(s)), None)),
        }
    }
}
...
pub struct WasiInputStream(InputStream);


impl Read for WasiInputStream {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        let r = self.0.read(54).unwrap();
...
impl Read for Reader {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match self { // add deref back
            Reader::Reader(ref mut rdr) => rdr.read(buf),
            Reader::Bytes(ref mut rdr) => rdr.read(buf),
            // Reader::Wasi(handle) => {
            //     let body_chunk = handle.read(buf.len() as u64).map_err(|_| io::Error::new(io::ErrorKind::Other, "read chunk"))?;
...
// #[derive(Debug)]
// struct WrappedInputStream {
//     pub inner: InputStream
// }

// impl Clone for WrappedInputStream {
//     fn clone(&self) -> WrappedInputStream {
//         WrappedInputStream {
//             inner: inner
//         }
//     }
